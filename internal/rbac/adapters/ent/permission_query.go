...
func (pq *PermissionQuery) QueryRoutes() *RouteQuery {
	query := &RouteQuery{config: pq.config}
...
func (pq *PermissionQuery) QueryNodes() *NodeQuery {
...
func (pq *PermissionQuery) WithRoutes(opts ...func(*RouteQuery)) *PermissionQuery {
	query := &RouteQuery{config: pq.config}
...
func (pq *PermissionQuery) WithNodes(opts ...func(*NodeQuery)) *PermissionQuery {
	query := &NodeQuery{config: pq.config}
	for _, opt := range opts {
		opt(query)
	}
...
//		GroupBy(permission.FieldTenant).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
//
...
func (pq *PermissionQuery) sqlAll(ctx context.Context) ([]*Permission, error) {
	...
	if query := pq.withRoutes; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		ids := make(map[int]*Permission, len(nodes))
		for _, node := range nodes {
			ids[node.ID] = node
			fks = append(fks, node.ID)
			node.Edges.Routes = []*Route{}
		}
		var (
			edgeids []int
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(permission.RoutesPrimaryKey[0], fks...))
			},
		...
		if err := sqlgraph.QueryEdges(ctx, pq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "routes": %w`, err)
		}
		query.Where(route.IDIn(edgeids...))
		...
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "routes" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Routes = append(nodes[i].Edges.Routes, n)
			}
		}
	}
	...
	if query := pq.withNodes; query != nil {
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(permission.NodesPrimaryKey[1], fks...))
			},
			ScanValues: func() [2]interface{} {
				return [2]interface{}{&sql.NullInt64{}, &sql.NullInt64{}}
			},
		...
		if err := sqlgraph.QueryEdges(ctx, pq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "nodes": %w`, err)
		}
		query.Where(node.IDIn(edgeids...))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		}
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "nodes" node returned %v`, n.ID)
			}
			for i := range nodes {
