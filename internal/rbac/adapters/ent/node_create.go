...
func (nc *NodeCreate) AddPermissionIDs(ids ...int) *NodeCreate {
	nc.mutation.AddPermissionIDs(ids...)
	return nc
}

// AddPermissions adds the "permissions" edges to the Permission entity.
func (nc *NodeCreate) AddPermissions(p ...*Permission) *NodeCreate {
	ids := make([]int, len(p))
	for i := range p {
		ids[i] = p[i].ID
	}
	return nc.AddPermissionIDs(ids...)
}

// AddUserIDs adds the "users" edge to the User entity by IDs.
func (nc *NodeCreate) AddUserIDs(ids ...int) *NodeCreate {
	nc.mutation.AddUserIDs(ids...)
	return nc
}

// AddUsers adds the "users" edges to the User entity.
func (nc *NodeCreate) AddUsers(u ...*User) *NodeCreate {
	ids := make([]int, len(u))
	for i := range u {
		ids[i] = u[i].ID
	}
...
func (nc *NodeCreate) createSpec() (*Node, *sqlgraph.CreateSpec) {
	...
	if nodes := nc.mutation.PermissionsIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2M,
			Inverse: false,
